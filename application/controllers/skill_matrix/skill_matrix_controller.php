...
class Skill_matrix_controller extends CI_Controller {
    ...
    function add_employee_skill_matrix() {
        ...
        $employee_skill_model->set_skill_cat_code($this->input->post('skill_cat_code', TRUE));
        $employee_skill_model->set_del_ind('1');
        $employee_skill_model->set_added_by($this->session->userdata('EMPLOYEE_CODE'));
